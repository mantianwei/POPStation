(function(){
    ...
    var colors = ['#FFFFFF', '#000000', '#3B3B3B', '#282828', '#5C5C5C'];
    function init() {
        initStages();
        initForm();
        initText();
    ...
    function tweenCircle(c, dir) {
        ...
        if(dir == 'in') {
            c.tween = TweenLite.to(c, 2, {x: c.originX, y: c.originY, ease:Quad.easeInOut, alpha: 1, radius: 5, scaleX: 0.4, scaleY: 0.4, onComplete: function() {
                c.movement = 'jiggle';
                tweenCircle(c);
            }});
        } else if(dir == 'out') {
            c.tween = TweenLite.to(c, 1, {x: window.innerWidth*Math.random(), y: window.innerHeight*Math.random(), ease:Quad.easeInOut, alpha: 0.2 + Math.random()*0.5, scaleX: 1, scaleY: 1, onComplete: function() {
                c.movement = 'float';
                tweenCircle(c);
            }});
        } else {
    ...
    function formText() {
        ...
        if(textPixels.length < circles.length) {
            for(var j = textPixels.length; j<circles.length; j++) {
                circles[j].tween = TweenLite.to(circles[j], 1.4, {alpha: 0.1});
            }
        }
    }
    ...
    function explode() {
        ...
        if(textPixels.length < circles.length) {
            for(var j = textPixels.length; j<circles.length; j++) {
                circles[j].tween = TweenLite.to(circles[j], 1.4, {alpha: 1});
            }
        }
    }
    function changeColor(){
        ...
        for(var i=0;i<circles.length;i++){
            ...
            circles[i].graphics.clear().beginFill(color).drawCircle(0, 0, 6).endFill();
        }
    }
    // event handlers
